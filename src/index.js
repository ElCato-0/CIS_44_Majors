import React from 'react';
import ReactDOM from 'react-dom/client';
import './index.css';
import App from './App';
import reportWebVitals from './reportWebVitals';
import {
  BrowserRouter,
  Route,
  Routes,
} from "react-router-dom";
import Home from './Home';
import BaseInfo from './BaseInfo';

const root = ReactDOM.createRoot(document.getElementById('root'));
root.render(
  <React.StrictMode>
    <BrowserRouter basename='CIS_44_Majors'>
      <App>
        <Routes>
          <Route path="/" element={<Home />} />
          <Route path="/cs" element={
            <BaseInfo
              image1='https://drive.uqu.edu.sa/_/cis/images/New%20programs/%D8%A8%D9%83%D8%A7%D9%84%D9%88%D8%B1%D9%8A%D9%88%D8%B3%20%D8%B9%D9%84%D9%88%D9%85%20%D8%A7%D9%84%D8%AD%D8%A7%D8%B3%D8%A8%20%D8%A7%D9%84%D8%A2%D9%84%D9%8A.jpg'
              image2='https://drive.uqu.edu.sa/_/cis/images/New%20programs/%D8%AE%D8%B7%D8%A9%20%D8%A8%D8%B1%D9%86%D8%A7%D9%85%D8%AC%20%D8%B9%D9%84%D9%88%D9%85%20%D8%A7%D9%84%D8%AD%D8%A7%D8%B3%D8%A8%20%D8%A7%D9%84%D8%A2%D9%84%D9%8A%20%D8%A7%D9%84%D9%85%D8%AD%D8%AF%D8%AB.jpg'
              data={[
                ['البرمجة الشيئية', 'دورة في مجال البرمجة تعلم الطلاب كيفية إنشاء برامج تتعامل مع الكائنات والمفاهيم المتعلقة بها. يتعلمون كيفية تصميم وتطوير تطبيقات قائمة على الكائنات باستخدام لغات البرمجة المناسبة.'],
                ['التصميم الرقمي والمنطقي', 'دورة تعلم الطلاب مبادئ التصميم الرقمي والمنطقي للأنظمة الإلكترونية. يتعلمون كيفية تحليل وتصميم الدوائر الرقمية واستخدام المنطق المنطقي في إنشاء الأنظمة الإلكترونية.'],
                ['الحاسب والمجتمع', 'دورة تعرض للطلاب دور الحاسوب وتأثيره على المجتمع والحياة اليومية. يتعلمون حول التطور التاريخي للحاسوب وتأثيره على المجالات المختلفة مثل التعليم والصناعة والاتصالات.'],

                ['مقدمة في الأمن السيبراني', 'دورة تعرض للطلاب مبادئ الأمان السيبراني وكيفية حماية الأنظمة والبيانات من التهديدات الإلكترونية. يتعلمون حول أساسيات الشبكات الآمنة والتشفير وتقنيات الكشف عن الاختراق.'],
                ['تنظيم وعمارة الحاسوب', 'دورة تعلم الطلاب عن تصميم وتنظيم أنظمة الحواسيب وهيكلية المعالج والذاكرة والمكونات الأساسية الأخرى. يتعلمون حول توزيع الموارد وتحسين أداء النظام ومفاهيم العمارة الحاسوبية.'],
                ['أسس هندسة البرمجيات', 'دورة تعرض للطلاب المفاهيم الأساسية لهندسة البرمجيات وعملية تطوير البرامج. يتعلمون عن مراحل التطوير وتقنيات إدارة المشروع واختبار البرمجيات وضمان الجودة.'],
                ['أساسيات قواعد البيانات', 'دورة تعلم الطلاب أساسيات قواعد البيانات وكيفية تنظيم وإدارة المعلومات. يتعلمون حول نماذج قواعد البيانات واستعلام البيانات باستخدام لغة الاستعلام المناسبة.'],

                ['تركيب المترجمات', 'دورة تعلم الطلاب عن تركيب المترجمات وعملية تحويل البرامج من لغة برمجة إلى لغة آلة قابلة للتنفيذ. يتعلمون حول المراحل المختلفة لترجمة البرامج وتحليل وتطبيق القواعد اللغوية.'],
                ['هندسة البرمجيات', 'دورة تعرض للطلاب مفاهيم هندسة البرمجيات وأساليبها في تطوير البرامج. يتعلمون عن تحليل المتطلبات وتصميم النظام واختبار البرمجيات وإدارة التغيير.'],
                ['إدارة المشاريع لعلماء الحاسوب', 'دورة تعلم الطلاب مهارات إدارة المشاريع في سياق علماء الحاسوب. يتعلمون حول التخطيط والتنظيم والتنفيذ والمراقبة والتقييم في إدارة مشاريع الحوسبة.'],
                ['أساسيات الخوازرميات', 'دورة تعلم الطلاب المفاهيم الأساسية للخوارزميات وكيفية حل المشكلات باستخدام الخوارزميات. يتعلمون حول التحليل الزمني والفضائي والخوارزميات الشائعة مثل البحث والفرز.'],
                ['أساسيات نظم التشغيل', 'دورة تعرض للطلاب مفاهيم نظم التشغيل ووظيفتها في إدارة الموارد وتشغيل البرامج. يتعلمون حول تنظيم العمليات وإدارة الذاكرة وجدولة المهام في نظم التشغيل.'],

                ['نظم التشغيل', 'دورة تعمق في مفاهيم نظم التشغيل وعملها. يتعلم الطلاب حول البنية الداخلية لنظم التشغيل وأساسيات تصميم وتطوير نظم التشغيل.'],
                ['الخوارزميات', 'دورة تعمق في مجال الخوارزميات وتحليل الأداء. يتعلم الطلاب عن خوارزميات متقدمة مثل البرمجة الديناميكية والترتيبات المتقدمة والبحث في الرسوم البيانية.'],
                ['توثيق البرمجيات والكتابة التقنية', 'دورة تعلم الطلاب كيفية كتابة التوثيق الفني والتقني للبرامج والأنظمة. يتعلمون حول كتابة المستندات الفنية والوثائق التقنية والتوثيق البرمجي.'],
                ['شبكات الحاسب والاتصالات', 'دورة تعرض للطلاب مفاهيم شبكات الحاسوب واتصالات البيانات. يتعلمون حول بنية الشبكات والبروتوكولات وتوصيل أجهزة الكمبيوتر في بيئة الشبكات.'],
                ['نظم قواعد البيانات', 'دورة تعلم الطلاب حول تصميم وإدارة نظم قواعد البيانات. يتعلمون حول نماذج البيانات المختلفة واستعلام البيانات وتنفيذ عمليات الإدخال والاسترجاع في قواعد البيانات.'],

                ['بناء نظم البرمجيات', 'دورة تعلم الطلاب كيفية بناء أنظمة برمجية معقدة وتطويرها. يتعلمون حول تحليل وتصميم النظام وتقنيات تنفيذ البرمجيات واختبارها وصيانتها.'],
                ['أساسيات تطوير الويب', 'دورة تعرض للطلاب أساسيات تطوير تطبيقات الويب. يتعلمون حول لغات البرمجة الشائعة في تطوير الويب وتصميم وتنفيذ صفحات الويب التفاعلية.'],
                ['برمجة متقدمة', 'دورة تعمق في مجال البرمجة وتقنياتها المتقدمة. يتعلم الطلاب عن تقنيات البرمجة المتقدمة مثل البرمجة الموجهة بالأحداث والبرمجة الوظيفية والبرمجة التفاعلية.'],
                ['مبادئ وتقنيات الذكاء الاصطناعي', 'دورة تعرض للطلاب مبادئ وتقنيات الذكاء الاصطناعي وتطبيقاتها في مجالات مختلفة. يتعلمون حول تصميم وتنفيذ نماذج الذكاء الاصطناعي وتحليل البيانات وتعلم الآلة.'],
                ['مقدمة في التشفير', 'دورة تعرض للطلاب مبادئ التشفير وأساليب حماية المعلومات. يتعلمون حول أساسيات التشفير وتقنيات تبادل المفاتيح وتوقيع الرسائل الرقمية.'],

                ['الحوسبة العلمية', 'دورة تعرض للطلاب استخدام الحوسبة في التطبيقات العلمية والحسابية. يتعلمون حول تطبيقات الحوسبة العلمية في الفيزياء والكيمياء والهندسة وعلوم الحاسوب الأخرى.'],
                ['تصميم واجهات المستخدم', 'دورة تعلم الطلاب مفاهيم تصميم واجهات المستخدم الجذابة والسهلة الاستخدام. يتعلمون حول تحليل احتياجات المستخدم وتصميم واجهات المستخدم واختبارها وتقييمها.'],
                ['تطوير تطبيقات الجوّال', 'دورة تعلم الطلاب تطوير تطبيقات الجوّال لأنظمة التشغيل المختلفة. يتعلمون حول تصميم وتنفيذ تطبيقات الجوّال القابلة للتوسع والتفاعلية.'],

                ["الحوسبة الموزعة والمتوازية", "دورة تعلم الطلاب حول مفاهيم الحوسبة الموزعة والمتوازية. يتعلمون حول تصميم وتطوير وإدارة نظم الحوسبة المتوازية والموزعة. تشمل المواضيع الأخرى للدورة تقنيات التوازي والتجزئة، وتوازن الحمل والاستجابة في الأنظمة الموزعة، وتواصل البيانات والتنسيق بين المكونات المختلفة في النظم المتوازية."],
                ["الرسومات بالحاسب", "في هذه الدورة، يتعلم الطلاب حول مفاهيم الرسومات الكمبيوترية وتقنياتها. يتعلمون حول تصميم وإنشاء الرسومات ثنائية الأبعاد وثلاثية الأبعاد وتطبيقها في البرامج وألعاب الكمبيوتر. يتعلمون أيضًا حول تقنيات الرسوم المتقدمة مثل الإضاءة والتظليل وتحريك الرسومات."]
              ]}
            />}
          />
          <Route path="/ce" element={
            <BaseInfo
              image1='https://drive.uqu.edu.sa/_/cis/images/New%20programs/%D8%A8%D9%83%D8%A7%D9%84%D9%88%D8%B1%D9%8A%D9%88%D8%B3%20%D9%87%D9%86%D8%AF%D8%B3%D8%A9%20%D8%A7%D9%84%D8%AD%D8%A7%D8%B3%D8%A8%20%D8%A7%D9%84%D8%A2%D9%84%D9%8A%20%D9%88%D8%A7%D9%84%D8%B4%D8%A8%D9%83%D8%A7%D8%AA.jpg'
              image2='https://drive.uqu.edu.sa/_/cis/images/New%20programs/%D8%AE%D8%B7%D8%A9%20%D8%A8%D8%B1%D9%86%D8%A7%D9%85%D8%AC%20%D9%87%D9%86%D8%AF%D8%B3%D8%A9%D8%A7%D9%84%D8%AD%D8%A7%D8%B3%D8%A8%20%D8%A7%D9%84%D8%A2%D9%84%D9%8A%20%D9%88%D8%A7%D9%84%D8%B4%D8%A8%D9%83%D8%A7%D8%AA.jpg'
              data={[

                ['التصميم الرقمي والمنطقي', 'دورة تعلم الطلاب مبادئ التصميم الرقمي والمنطقي للأنظمة الإلكترونية. يتعلمون كيفية تحليل وتصميم الدوائر الرقمية واستخدام المنطق المنطقي في إنشاء الأنظمة الإلكترونية.'],

                ["الفيزياء للمهندسين", "دورة تعلم الطلاب مبادئ الفيزياء الهندسية وتطبيقاتها في الهندسة. يتعلمون حول الحركة والقوى والطاقة والحرارة والموجات والصوت والضوء والكهرباء والمغناطيسية والنظرية النسبية والكمية."],

                ["اساسيات الشبكات وتطبيقاتها", "دورة تعلم الطلاب مبادئ الشبكات وتطبيقاتها في الحياة العملية. يتعلمون حول تصميم وتنفيذ وإدارة الشبكات والاتصالات والأنظمة اللاسلكية والأمن السيبراني."],
                
                ["كيمياء عامة", ""],

                ["تنسيق الحاسب الالي", ""],

                ["نظرية الدوائر", "دورة نظرية الدوائر تعرض للطلاب المفاهيم الأساسية في مجال نظرية الدوائر الإلكترونية. يتعلم الطلاب حول التصميم والتحليل والتشغيل للدوائر الإلكترونية المنطقية. يتضمن ذلك دراسة المكونات الأساسية للدوائر مثل البوابات المنطقية والوصلات والمتغيرات والمقاومات. يتعلم الطلاب كيفية استخدام قوانين الجبر البولي لتحليل وتصميم الدوائر الرقمية. تتضمن الدورة أيضًا دراسة الدوائر المتكاملة وتصميم الدوائر المنطقية المتكاملة وتطبيقاتها في المجالات المختلفة."],
                ["عمارة الحاسب الالي", "مفاهيم تصميم وهندسة الحواسيب. يتعلم الطلاب حول تركيب وتنظيم مكونات الحاسب الآلي، بما في ذلك المعالج والذاكرة والتخزين ووحدات الإدخال والإخراج. يتعرفون على بنية الحاسب الآلي وكيفية تفاعل المكونات معًا لتنفيذ العمليات الحسابية وتخزين البيانات. تشمل المحتويات الأخرى للدورة مواضيع مثل مخططات الدوائر الداخلية، وتقنيات الاتصال بين المكونات، وتحليل أداء النظام، وتصميم الشبكات الداخلية للحواسيب."],

                ["تصميم النظر الرقمية", "دورة تعلم الطلاب مفاهيم تصميم النظم الرقمية. يتعلمون حول تصميم وتحليل الدوائر الرقمية والأنظمة المنطقية. يتضمن ذلك دراسة تصميم العتاد الرقمي والأدوات والتقنيات المستخدمة فيه، وكذلك تصميم الدوائر المنطقية المتكاملة والاستخدامات العملية لها في مجالات مثل المعالجات الرقمية والشبكات وأنظمة الاتصالات."],

                ["هندسة الشبكات", "دورة تعلم الطلاب مفاهيم هندسة الشبكات وتصميم وإدارة الشبكات المختلفة. يتعلمون حول تكنولوجيات الشبكات، بروتوكولات الاتصال، وتأمين الشبكات. يتعلمون أيضًا حول التخطيط والتصميم والتشغيل والصيانة للشبكات السلكية واللاسلكية."],

                ["رياضيات هندسية متقدمة", "دورة تعمق في مجال الرياضيات الهندسية المتقدمة. يتعلم الطلاب عن المفاهيم الرياضية المتقدمة المستخدمة في التصميم والتحليل الهندسي. يتضمن ذلك دراسة الأشكال الهندسية المتقدمة، والتحويلات الهندسية، والتفاضل والتكامل في البُعد العالي، والمعادلات التفاضلية العادية."],

                ["دوائر الكترونية", "دورة تعلم الطلاب عن مفاهيم وتصميم دوائر الكترونية. يتعلمون حول العناصر الأساسية للدوائر الإلكترونية وكيفية تجميعها وتشغيلها. يتضمن ذلك دراسة العناصر النشطة والمقاومات والمكثفات والملفات والبوابات المنطقية. يتعلم الطلاب أيضًا حول تصميم الدوائر الإلكترونية المبدئي وتحليلها باستخدام الأدوات والتقنيات المناسبة."],

                ["الإشارات والنظم", "دورة تعلم الطلاب حول مفاهيم الإشارات والنظم. يتعلمون حول تحليل وتصميم الإشارات والأنظمة المختلفة، وكيفية تمثيلها ومعالجتها. تشمل المواضيع التي يتم تغطيتها الإشارات الأنالوجية والرقمية، وعمليات التحويل بينهما، وتحليل التردد والزمن للإشارات، وتصميم وتطبيق الأنظمة المرشحة والمعالجة الرقمية للإشارات."],
                ["أنظمة اتصال الحاسب", "دورة تعلم الطلاب حول مفاهيم أنظمة اتصال الحاسب. يتعلمون حول التصميم والتشغيل والصيانة لأنظمة الاتصال والشبكات المستخدمة في الحوسبة. تشمل المواضيع التي يتم تغطيتها البروتوكولات، وتقنيات الاتصال، وتكنولوجيا الشبكات المختلفة. يتعلم الطلاب أيضًا حول أمان الاتصالات وحماية البيانات في أنظمة الاتصال الحاسوبية."],
                ["أساسيات نظم التشغيل", "دورة تعلم الطلاب المفاهيم الأساسية في نظم التشغيل. يتعلمون حول بنية ووظيفة نظم التشغيل وكيفية إدارة الموارد وتنظيم العمليات وإدارة الذاكرة وتخطيط الجدولة. تشمل المحتويات الأخرى للدورة الملفات، والإدخال/الإخراج، والتحكم في الوصول إلى النظام، والأمان وحماية النظام."],
                ["المهارات الاحترافية للتصميم الهندسي", "دورة تعلم الطلاب المهارات الاحترافية اللازمة للتصميم الهندسي. يتعلمون حول تطوير الفهم والقدرات العملية في مجالات التصميم الهندسي، بما في ذلك التصميم الإبداعي والتفكير النقدي وتطوير المشاريع. يتضمن ذلك التعرف على أدوات وتقنيات التصميم، وتطوير المهارات في التواصل وإدارة المشاريع، والعمل الجماعي وحل المشكلات."],
                ["إدارة الشبكات", "دورة تعلم الطلاب حول مفاهيم إدارة الشبكات. يتعلمون حول التخطيط وتصميم وتنفيذ وصيانة وتشغيل الشبكات المختلفة. يشمل المحتوى الدراسي للدورة البنية الفيزيائية والمنطقية للشبكات، وتكنولوجيات الشبكات، وإدارة الموارد والأمان وتحليل الأداء. يتعلم الطلاب أيضًا حول حل المشاكل واستكشاف الأخطاء وإصلاحها في الشبكات."],
                ["مبادئ الأمان السيبراني", "دورة تعلم الطلاب مبادئ الأمان السيبراني. يتعلمون حول تهديدات الأمان السيبراني والضوابط والتدابير الوقائية لحماية الأنظمة والبيانات. يشمل المحتوى الدراسي للدورة الاستشعار والاستجابة للتهديدات، وتحليل الثغرات والهجمات، والتشفير والتوقيع الرقمي، وتقنيات الحماية وإدارة الأمان السيبراني."],
                ["أنظمة إلكترونية رقمية ودوائر", "دورة تعلم الطلاب حول مفاهيم أنظمة إلكترونية رقمية والدوائر الإلكترونية. يتعلمون حول التصميم والتحليل والتشغيل والاختبار للدوائر الإلكترونية المختلفة. تشمل المحتويات الأخرى للدورة العناصر الإلكترونية الأساسية، ومفاهيم التشغيل والاختبار، وتصميم الدوائر المنطقية والتركيبات المنطقية والمراقبة والتحكم."],
                ["أساسيات قواعد البيانات", "دورة تعلم الطلاب المفاهيم الأساسية في قواعد البيانات. يتعلمون حول تصميم وإدارة قواعد البيانات واستخدامها في تخزين واسترجاع البيانات. تشمل المحتويات الأخرى للدورة تعريف قواعد البيانات، وتصميم جداول البيانات، والاستعلامات والتحديثات والإدخالات والإخراجات."],
                ["الطرق العددية للحساب", "دورة تعلم الطلاب حول المفاهيم الأساسية للطرق العددية في الحساب. يتعلمون حول تقدير الحلول العددية للمعادلات والتفاضلات والمسائل الرياضية المعقدة. تشمل المواضيع الأخرى للدورة التقريب والتكامل العددي، وحل الأنظمة الخطية، والتقنيات العددية في حساب التفاضلات والتكاملات."],
                ["برمجة الشبكات", "دورة تعلم الطلاب حول برمجة الشبكات وتطوير تطبيقات الشبكات. يتعلمون حول تصميم وتطوير وتنفيذ برامج الشبكات والتطبيقات المرتبطة بها. تشمل المواضيع الأخرى للدورة البروتوكولات والتقنيات المستخدمة في برمجة الشبكات، وتحليل البيانات واستخدام واجهات برمجة التطبيقات للتواصل بين الأجهزة والخوادم."],
                ["المعالجات الدقيقة", "دورة تعلم الطلاب حول مفاهيم المعالجات الدقيقة. يتعلمون حول تصميم وتشغيل المعالجات الدقيقة وتحليل أدائها وتحسينه. تشمل المواضيع الأخرى للدورة بنية المعالج، ونماذج التنفيذ، ومجموعات التعليمات وتنفيذ البرامج، وتحليل الأداء والتحسينات."],
                ["إدارة المشاريع التقنية", "دورة تعلم الطلاب حول إدارة المشاريع التقنية. يتعلمون حول تخطيط وتنفيذ ومتابعة المشاريع التقنية بكفاءة. تشمل المواضيع الأخرى للدورة إدارة الموارد والجداول الزمنية والتكاليف والتواصل وحل المشاكل وإدارة المخاطر في المشاريع التقنية."],
                ["تصميم أنظمة الحاسبات الصغيرة", "دورة تعلم الطلاب حول تصميم أنظمة الحاسبات الصغيرة. يتعلمون حول تحليل وتصميم وتنفيذ الأنظمة الصغيرة التي تستخدم في بيئات صغيرة مثل الأجهزة المحمولة والمدمجة. تشمل المواضيع الأخرى للدورة تصميم الأجهزة والبرمجيات المختلفة والتواصل بين الأجهزة والمستخدمين."],
                ["عملية وأدوات التصميم الهندسي", "دورة تعلم الطلاب عن العملية والأدوات المستخدمة في التصميم الهندسي. يتعلمون حول مراحل التصميم والمفاهيم الأساسية والتقنيات المستخدمة في عملية التصميم. تشمل المواضيع الأخرى للدورة تحليل المتطلبات، وتصميم المفهوم، والنمذجة والمحاكاة، والتوثيق والتحليل الاقتصادي للتصميمات."],
              ]}
            />}
          />
          <Route path="/se" element={
            <BaseInfo
              image1='https://drive.uqu.edu.sa/_/cis/images/New%20programs/%D8%A8%D9%83%D8%A7%D9%84%D9%88%D8%B1%D9%8A%D9%88%D8%B3%20%D9%87%D9%86%D8%AF%D8%B3%D8%A9%20%D8%A7%D9%84%D8%A8%D8%B1%D9%85%D8%AC%D9%8A%D8%A7%D8%AA.jpg'
              image2='https://drive.uqu.edu.sa/_/cis/images/New%20programs/%D8%AE%D8%B7%D8%A9%20%D8%A8%D8%B1%D9%86%D8%A7%D9%85%D8%AC%20%D9%87%D9%86%D8%AF%D8%B3%D8%A9%20%D8%A7%D9%84%D8%A8%D8%B1%D9%85%D8%AC%D9%8A%D8%A7%D8%AA.jpg'
              data={[
                ['أسس هندسة البرمجيات', 'دورة تعلم الطلاب حول أساسيات هندسة البرمجيات. يتعلمون حول عملية تطوير البرمجيات ومفاهيمها الأساسية مثل التحليل، والتصميم، والتنفيذ، والاختبار. تشمل المواضيع الأخرى للدورة النماذج والأساليب المستخدمة في هندسة البرمجيات، وإدارة مشاريع البرمجيات، وتوثيق البرمجيات.'],
                ['واجهات بين البرامج والأجهزة', 'دورة تعلم الطلاب حول تصميم وتطوير واجهات بين البرامج والأجهزة. يتعلمون حول كيفية ربط وتفاعل البرامج مع الأجهزة المختلفة مثل الحواسيب، والأجهزة الذكية، والمستشعرات. تشمل المواضيع الأخرى للدورة تقنيات الاتصال وبروتوكولات الاتصال المستخدمة في واجهات البرامج والأجهزة.'],
                ['متطلبات البرمجيات 1', 'دورة تعلم الطلاب حول مفهوم وتحليل وتوثيق متطلبات البرمجيات. يتعلمون كيفية فهم وتحليل احتياجات المستخدمين وتوثيقها في صورة متطلبات قابلة للتنفيذ. تشمل المواضيع الأخرى للدورة تقنيات جمع المتطلبات، وتحليل المتطلبات، وتوثيق المتطلبات.'],
                ['البرمجة الموزعة', 'دورة تعلم الطلاب حول تطوير التطبيقات الموزعة والتعامل مع التحديات المرتبطة بها. يتعلمون حول استخدام تقنيات وأدوات البرمجة الموزعة وتصميم وتنفيذ التطبيقات التي تعمل عبر شبكات متعددة. تشمل المواضيع الأخرى للدورة التعامل مع الاتصال والتنسيق بين المكونات الموزعة، وإدارة التحكم في العمليات الموزعة.'],
                ['متطلبات البرمجيات 2', 'دورة تعلم الطلاب حول تحليل وتصميم وتوثيق متطلبات البرمجيات المتقدمة. يتعلمون حول تقنيات تحليل وتصميم المتطلبات المتقدمة، والتعامل مع تعقيدات المشاريع البرمجية. تشمل المواضيع الأخرى للدورة تحليل القيود والاعتبارات الأخلاقية والاجتماعية لتطوير البرمجيات.'],
                ['هندسة الويب', 'دورة تعلم الطلاب حول تصميم وتطوير تطبيقات الويب. يتعلمون حول تقنيات بناء وتصميم صفحات الويب وتطوير تطبيقات الويب الديناميكية باستخدام لغات الويب المختلفة مثل HTML، CSS، JavaScript. تشمل المواضيع الأخرى للدورة تصميم قواعد البيانات لتطبيقات الويب وتجربة المستخدم وأمان المواقع الإلكترونية.'],
                ['إدارة المشاريع', 'دورة تعلم الطلاب حول إدارة المشاريع التقنية. يتعلمون كيفية التخطيط والتنظيم والتنسيق لتنفيذ مشاريع تقنية بنجاح. تشمل المواضيع الأخرى للدورة تحديد أهداف المشروع، وجدولة المهام، وتخطيط الموارد، وإدارة المخاطر.'],
                ['اختبار البرمجيات', 'دورة تعلم الطلاب حول عملية اختبار البرمجيات. يتعلمون كيفية تطوير استراتيجيات وتقنيات اختبار البرمجيات وتنفيذها لضمان جودة وكفاءة التطبيقات البرمجية. تشمل المواضيع الأخرى للدورة تحليل متطلبات الاختبار، وتصميم حالات الاختبار، وتنفيذ الاختبار، وتقييم النتائج.'],
                ['هندسة تطبيقات الجوال', 'دورة تعلم الطلاب حول تصميم وتطوير تطبيقات الجوال لأنظمة التشغيل المختلفة مثل Android وiOS. يتعلمون حول استخدام أدوات التطوير المختلفة ولغات البرمجة لبناء تطبيقات الجوال التفاعلية والمتقدمة. تشمل المواضيع الأخرى للدورة تصميم واجهات المستخدم للتطبيقات الجوال وتكامل الميزات المختلفة.'],
                ['النماذج والطرق', 'دورة تعلم الطلاب حول النماذج والأساليب المستخدمة في هندسة البرمجيات. يتعلمون حول النماذج المختلفة لتطوير البرمجيات مثل نموذج البنية، ونموذج النمط، ونموذج القائمة. تشمل المواضيع الأخرى للدورة تقنيات التحليل والتصميم والتطبيق للنماذج المختلفة.'],
                ['تصميم وإنشاء البرمجيات 2', 'دورة تعلم الطلاب حول تصميم وتنفيذ البرمجيات المتقدمة. يتعلمون كيفية تحليل وتصميم وتطوير البرمجيات الكبيرة والمعقدة باستخدام تقنيات وأدوات متقدمة. تشمل المواضيع الأخرى للدورة تصميم قواعد البيانات المتقدمة وتحليل الأداء وتطبيق مفاهيم تنمية البرمجيات المستدامة.'],
                ['الرياضيات لهندسة البرمجيات', 'دورة تعلم الطلاب حول الرياضيات وتطبيقاتها في هندسة البرمجيات. يتعلمون حول المفاهيم الرياضية المستخدمة في تحليل وتصميم البرمجيات، مثل الجبر الخطي، والإحصاء، ونظرية الاحتمالات. تشمل المواضيع الأخرى للدورة تحليل الأداء والتحليل العددي والتحسين الرياضي للبرامج.'],
                ['طرق البحث', 'دورة تعلم الطلاب حول طرق البحث في هندسة البرمجيات. يتعلمون المنهجيات والأدوات المستخدمة في إجراء البحث وجمع البيانات وتحليلها للوصول إلى نتائج موثوقة. تشمل المواضيع الأخرى للدورة تصميم التجارب والاستنتاجات الإحصائية وكتابة التقارير العلمية.'],
                ['صيانة وتحديث البرمجيات', 'دورة تعلم الطلاب حول صيانة وتحديث البرمجيات المستدامة. يتعلمون كيفية تحليل وتقييم وتحديث البرمجيات الحالية للحفاظ على جودتها وتحسينها مع مرور الوقت. تشمل المواضيع الأخرى للدورة إدارة الإصدارات وإصلاح الأخطاء وترقية التقنيات.'],
                ['مقدمة في الأمن السيبراني', 'دورة تعلم الطلاب حول مبادئ ومفاهيم الأمن السيبراني. يتعلمون حول التهديدات والهجمات السيبرانية وكيفية حماية البرمجيات والبيانات منها. تشمل المواضيع الأخرى للدورة تحليل الضعف وتطبيق إجراءات الأمان والتعامل مع الحوادث الأمنية.'],
                ['الحوسبة العلمية', 'دورة تعلم الطلاب حول استخدام الحوسبة في تطبيقات العلوم والهندسة. يتعلمون حول الأسس الرياضية والنمذجة العددية والمحاكاة باستخدام أدوات الحوسبة. تشمل المواضيع الأخرى للدورة تحليل البيانات العلمية وتحسين الأداء والتطبيقات العملية.'],
                ['الأطر والأدوات', 'دورة تعلم الطلاب حول الأطر والأدوات المستخدمة في هندسة البرمجيات. يتعلمون حول الأطر البرمجية والأدوات التطوير المستخدمة في تطوير واختبار البرمجيات. تشمل المواضيع الأخرى للدورة تعلم استخدام أدوات إدارة المشاريع وأنظمة التحكم في الإصدارات وتطبيقات تطوير الويب.'],
                ['جودة البرمجيات', 'دورة تعلم الطلاب حول مفهوم وتحقيق جودة البرمجيات. يتعلمون حول الأسس النظرية لضمان جودة البرمجيات وتقنيات التحقق والاختبار والتحليل. تشمل المواضيع الأخرى للدورة تحسين العمليات وإدارة الأخطاء وقياس الأداء.'],
                ['هندسة البيانات', 'دورة تعلم الطلاب حول استخدام البيانات في هندسة البرمجيات. يتعلمون كيفية تحليل وتخزين واسترجاع وتحليل البيانات الكبيرة وتطبيقات التعلم الآلي وتعدين البيانات. تشمل المواضيع الأخرى للدورة تصميم نماذج بيانات فعالة وتطبيق تقنيات البيانات المتقدمة.'],
                ['بحوث العمليات', 'دورة تعلم الطلاب حول تطبيقات بحوث العمليات في هندسة البرمجيات. يتعلمون حول تحليل المشاكل وتصميم النماذج الرياضية واستخدام الأدوات والتقنيات لاتخاذ قرارات فعالة. تشمل المواضيع الأخرى للدورة تحليل البيانات وتحسين العمليات وتطبيقات بحوث العمليات في الصناعة والخدمات.'],
                ['إدارة هندسة البرمجيات', 'دورة تعلم الطلاب حول إدارة عمليات هندسة البرمجيات. يتعلمون حول التخطيط والتنظيم والرقابة على عمليات تطوير البرمجيات وتنفيذها بكفاءة. تشمل المواضيع الأخرى للدورة إدارة المشاريع وتقييم الأداء وتطبيقات إدارة هندسة البرمجيات في المشاريع الحقيقية.'],
                ['الممارسات المهنية للمهندس', 'دورة تعلم الطلاب حول الممارسات المهنية والأخلاقية في مجال هندسة البرمجيات. يتعلمون حول المسؤولية الاجتماعية والقانونية للمهندسين ومعايير الجودة والأمان والخصوصية. تشمل المواضيع الأخرى للدورة تطبيق مبادئ المهندسين المحترفين والتعامل مع الصراعات المهنية وتحسين الأداء المهني.']
              ]}
            />}
          />
          <Route path="/sec" element={
            <BaseInfo
              image1='https://drive.uqu.edu.sa/_/cis/images/New%20programs/%D8%A8%D9%83%D8%A7%D9%84%D9%88%D8%B1%D9%8A%D9%88%D8%B3%20%D8%A7%D9%84%D8%A3%D9%85%D9%86%20%D8%A7%D9%84%D8%B3%D9%8A%D8%A8%D8%B1%D8%A7%D9%86%D9%8A.jpg'
              image2='https://drive.uqu.edu.sa/_/cis/images/New%20programs/%D8%AE%D8%B7%D8%A9%20%D8%A8%D8%B1%D9%86%D8%A7%D9%85%D8%AC%20%D8%A7%D9%84%D8%A3%D9%85%D9%86%20%D8%A7%D9%84%D8%B3%D9%8A%D8%A8%D8%B1%D8%A7%D9%86%D9%8A.jpg'
              data={[
                ['name', 'des'],
              ]}
            />}
          />
          <Route path="/ds" element={
            <BaseInfo
              image1='https://drive.uqu.edu.sa/_/cis/images/New%20programs/%D8%A8%D9%83%D8%A7%D9%84%D9%88%D8%B1%D9%8A%D9%88%D8%B3%20%D8%B9%D9%84%D9%85%20%D8%A7%D9%84%D8%A8%D9%8A%D8%A7%D9%86%D8%A7%D8%AA.jpg'
              image2='https://drive.uqu.edu.sa/_/cis/images/New%20programs/%D8%AE%D8%B7%D8%A9%20%D8%A8%D8%B1%D9%86%D8%A7%D9%85%D8%AC%20%D8%B9%D9%84%D9%85%20%D8%A7%D9%84%D8%A8%D9%8A%D8%A7%D9%86%D8%A7%D8%AA.jpg'
              data={[
                ['name', 'des'],
              ]}
            />}
          />
          <Route path="/ai" element={
            <BaseInfo
              image1='https://drive.uqu.edu.sa/_/cis/images/New%20programs/%D8%A8%D9%83%D8%A7%D9%84%D9%88%D8%B1%D9%8A%D9%88%D8%B3%20%D8%A7%D9%84%D8%B0%D9%83%D8%A7%D8%A1%20%D8%A7%D9%84%D8%A7%D8%B5%D8%B7%D9%86%D8%A7%D8%B9%D9%8A.jpg'
              image2='https://drive.uqu.edu.sa/_/cis/images/New%20programs/%D8%AE%D8%B7%D8%A9%20%D8%A8%D8%B1%D9%86%D8%A7%D9%85%D8%AC%20%D8%A7%D9%84%D8%B0%D9%83%D8%A7%D8%A1%20%D8%A7%D9%84%D8%A7%D8%B5%D8%B7%D9%86%D8%A7%D8%B9%D9%8A.jpg'
              data={[
                ['name', 'des'],
              ]}
            />}
          />
          <Route path="/hci" element={
            <BaseInfo
              image1='https://drive.uqu.edu.sa/_/cis/images/New%20programs/%D8%A8%D9%83%D8%A7%D9%84%D9%88%D8%B1%D9%8A%D9%88%D8%B3%20%D8%AA%D9%81%D8%A7%D8%B9%D9%84%20%D8%A7%D9%84%D8%A5%D9%86%D8%B3%D8%A7%D9%86%20%D9%85%D8%B9%20%D8%A7%D9%84%D8%AD%D8%A7%D8%B3%D8%A8.jpg'
              image2='https://drive.uqu.edu.sa/_/cis/images/New%20programs/%D8%AE%D8%B7%D8%A9%20%D8%A8%D8%B1%D9%86%D8%A7%D9%85%D8%AC%20%D8%AA%D9%81%D8%A7%D8%B9%D9%84%20%D8%A7%D9%84%D8%A5%D9%86%D8%B3%D8%A7%D9%86%20%D9%85%D8%B9%20%D8%A7%D9%84%D8%AD%D8%A7%D8%B3%D8%A8.jpg'
              data={[
                ['name', 'des'],
              ]}
            />}
          />
        </Routes>
      </App>
    </BrowserRouter>
  </React.StrictMode>
);

// If you want to start measuring performance in your app, pass a function
// to log results (for example: reportWebVitals(console.log))
// or send to an analytics endpoint. Learn more: https://bit.ly/CRA-vitals
reportWebVitals();
